...
"""
    ...
    def _convert_fitted_to_dataframe(self, fitted_peaks):
        """
        Convert fitted_peaks (list of dicts) to peak_list (DataFrame)
        ...
        print(f"   🔄 Converted {len(fitted_peaks)} fitted_peaks → peak_list DataFrame")
        return df
...
#
        if detected_peaks is not None and (
            ...
            # CRITICAL: Replace reference peak_list with detected positions
            # This ensures fitting uses DETECTED positions (red dots), not REFERENCE positions (blue crosses)
            if self.integrator.fitted_peaks:
                # Backup original reference peak_list
                if not hasattr(self.integrator, 'peak_list_original'):
                    self.integrator.peak_list_original = self.integrator.peak_list.copy()
                    print(f"   💾 Backed up original reference peak_list ({len(self.integrator.peak_list_original)} peaks)")

                # Convert fitted_peaks → peak_list DataFrame
                ...
                if detected_df is not None:
                    ...
                    print(f"   ✅ peak_list now contains DETECTED positions (fitting will use red dots, not blue crosses)")
    ...
    def _detect_peaks_sn_native(self):
        ...
        try:
            ...
            if detected_peaks and len(detected_peaks) > 0:
                ...
                # CRITICAL: Convert fitted_peaks → peak_list for consistency
                # S/N mode doesn't have a reference list, but we create peak_list from detected peaks
                if self.integrator.fitted_peaks:
                    detected_df = self._convert_fitted_to_dataframe(self.integrator.fitted_peaks)
                    if detected_df is not None:
                        self.integrator.peak_list = detected_df
                        print(f"   ✅ Created peak_list from {len(self.integrator.fitted_peaks)} S/N detected peaks")
    ...
    def _complete_new_single_spectrum_fitting(self, summary, progress_dialog):
        ...
        if summary['results']:
            ...
            for result in summary['results']:
                ...
                # Add bidirectional quality field mapping for visualization compatibility
            ...
            if hasattr(self, 'peak_navigator') and self.peak_navigator:
                ...
                print(f"🔧 Standardized {len(standardized_for_navigator)} results for Peak Navigator")
                if standardized_for_navigator:
                    print(f"   Sample result keys: {list(standardized_for_navigator[0].keys())}")
                    print(f"   Sample assignment: {standardized_for_navigator[0].get('assignment', 'N/A')}")
                    print(f"   Sample height: {standardized_for_navigator[0].get('height', 'N/A')}")
                    print(f"   Sample r_squared: {standardized_for_navigator[0].get('r_squared', 'N/A')}")
                    print(f"   Sample fitted: {standardized_for_navigator[0].get('fitted', 'N/A')}")
        ...
        # Update main spectrum plot to show color-coded peaks
        self.update_main_plot()
    ...
    def _complete_new_multi_spectrum_processing(self, new_batch_results, progress_dialog):
        ...
        try:
            ...
            # NOTE: Skip main plot update after series integration - no need to refresh
            # the main spectrum plot when viewing series overview. Avoids state mismatch
            # between integrator data (last spectrum) and fitted peaks (all spectra).
            # self._safe_update_main_plot()  # Disabled for series integration
    ...
    def _convert_multispec_to_legacy_format(self, new_batch_results):
        ...
        def get_summary():
            ...
            return {
                ...
                'duration': summary.get('duration', 'Unknown'),  # Fix KeyError: 'duration'
                'processing_mode': summary.get('processing_mode', 'series_integration')
            }
...
═══════════════════════════════
    ...
    def _sync_results_to_legacy_components(self, new_batch_results):
        ...
        # CRITICAL: Use integration_results (standardized format) instead of fitted_results (raw format)
        latest_spectrum_results = None
        for spectrum_name, spectrum_data in new_batch_results['results'].items():
            if isinstance(spectrum_data, dict) and spectrum_data.get('success', False):
                # Try integration_results first (standardized format with proper keys)
                integration_results = spectrum_data.get('integration_results', [])
                if integration_results:
                    latest_spectrum_results = integration_results
                    print(f"   📋 Using integration_results from {spectrum_name}: {len(integration_results)} peaks")
                    break  # Use first successful spectrum
                # Fallback to fitted_results if integration_results not available
                fitted_results = spectrum_data.get('fitted_results', [])
                if fitted_results:
                    latest_spectrum_results = fitted_results
                    print(f"   📋 Using fitted_results from {spectrum_name}: {len(fitted_results)} peaks")
                    break
        ...
        if latest_spectrum_results:
            ...
            if hasattr(self, 'peak_navigator') and self.peak_navigator:
                try:
                    print(f"   🔄 Updating Peak Navigator with {len(latest_spectrum_results)} peaks")
